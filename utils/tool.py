...
def user_sanitize_info(info):
    ...
    for formats in info['formats']:
        if 'none' in formats['video_ext'] and 'none' != formats['audio_ext']:
            file_size_sanitize = formats.get('filesize')
            format_note_sanitize = formats.get('format_note', '')
            user_dict['audio_formats'].append(
                {
                    'format_name': formats['format'],
                    'url': formats['url'],
                    'audio_ext': formats['audio_ext'],
                    'format_note': format_note_sanitize,
                    'file_size': size(file_size_sanitize) if file_size_sanitize is not None else 'N/A'
                }
            )
...
# info = get_youtube_extract('https://www.youtube.comc/watch?v=_XYHjf25GP')
